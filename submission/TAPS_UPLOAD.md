...
The paper text is tables-only; no `\includegraphics` calls are issued. The TAPS ZIP therefore contains only `vgac_pearc.tex`, `vgac_pearc.bbl`, and `refs.bib`. Figures referenced in the artifact (`figures/`) live in the GitHub repo and the Zenodo deposit, not in the TAPS upload, since TAPS does not need them for typesetting.
