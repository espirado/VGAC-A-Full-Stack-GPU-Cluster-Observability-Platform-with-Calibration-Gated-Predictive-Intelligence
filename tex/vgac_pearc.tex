...
\title{From Calibrated Probabilities to Scheduling Decisions:
...
\label{sec:intro}
A growing body of work has demonstrated that queue-delay predictions can be made calibrated---that $\hat{p}=P(\text{delay} > T)$ can be made to match realized frequencies~\cite{guo2017calibration}. But a calibrated probability alone does not tell a scheduler what to do. The missing piece is a decision-theoretic bridge: formal rules that translate $\hat{p}$ into actions, with explicit prerequisites on how calibrated $\hat{p}$ must be---and on how that calibration is monitored---for each action to be trustworthy.
